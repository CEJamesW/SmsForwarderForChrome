...
- 支持SIM卡槽位选择（双卡用户）
- 自动提取短信中的 4–8 位验证码，并填入当前网页的验证码控件
- 仅验证码短信显示系统通知；积分、营销等普通短信只保留在侧边栏，不弹右下角通知
- 支持普通与分段验证码输入框、动态表单、iframe 和常见 React/Vue 表单
- 从 SmsForwarder SIM 信息自动识别收信手机号及国家区号（如 `country_iso=cn` → `+86`）
- 在手机号输入框的右键菜单中手动触发填入，并智能匹配国家区号输入框或下拉框
...
1. 在设置页配置服务器后点击「刷新配置」，让扩展读取 SIM 号码、卡槽和 `country_iso`。
2. 保持「验证码自动填充」和「从 SIM 信息自动识别」开启。
3. 在网页的手机号输入框上点击右键，选择「填入手机号（自动匹配区号）」；如果页面有独立区号控件，会自动选择对应区号，否则只填不带区号的手机号。
4. 新验证码短信到达后，扩展会提取验证码并填入当前活动页面。

手机号不会自动填入，扩展也不会点击“登录”“注册”或“提交”。SIM 信息不可用或使用双卡时，可在设置页填写备用手机号和备用国家区号。
